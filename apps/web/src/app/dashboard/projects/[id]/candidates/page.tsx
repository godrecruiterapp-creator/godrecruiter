        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search candidates…" className="h-8 w-52 pl-8 pr-7 text-sm" />
          ...
          {['Available Today', 'No Contact 7d', 'Healthcare', 'Top Match'].map(f => (
            <button key={f} className="h-7 px-2.5 text-sm border border-dashed border-border rounded-full text-muted-foreground hover:border-foreground hover:text-foreground transition-colors whitespace-nowrap">
              {f}
            </button>
          ))}
        </div>
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 px-5 py-2 bg-accent/60 border-b shrink-0 flex-wrap">
          ...
          {[
            ...
            <Button key={a.label} size="sm" variant="outline" className="h-7 text-sm gap-1"><a.icon className="size-3" />{a.label}</Button>
          ))}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="sm" variant="outline" className="h-7 text-sm gap-1">More <ChevronDown className="size-3" /></Button>
            </DropdownMenuTrigger>
        ...
        {KANBAN_STAGES.map(stage => (
          <div key={stage} className="flex flex-col gap-2 w-56 shrink-0 h-full">
            <div className="flex items-center justify-between px-1 shrink-0">
              ...
              <span className="text-sm text-muted-foreground font-medium">{byStage[stage]?.length ?? 0}</span>
            </div>
