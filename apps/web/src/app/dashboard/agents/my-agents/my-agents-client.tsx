...
export function MyAgentsClient({ agents }: { agents: Agent[] }) {
  ...
  const sorted = useMemo(() => sortKey ? [...filtered].sort((a, b) => {
    const get = (ag: Agent): string => {
      ...
      if (sortKey === 'success_rate') return String(ag.success_rate ?? -1)
      return ''
    }
  ...
  function renderCell(key: ColKey, a: Agent) {
    switch (key) {
      ...
      case 'success_rate': return a.success_rate === null
        ? <span className="table-cell-secondary text-muted-foreground">—</span>
        : <span className={cn('table-cell-secondary tabular-nums', rateColor(a.success_rate))}>{a.success_rate}%</span>
          ...
          <div className="flex-1 overflow-auto">
            {agents.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 py-20">
                <div className="size-12 rounded-xl bg-brand-muted flex items-center justify-center">
                  <Bot className="size-6 text-brand" />
                </div>
                <div className="text-center">
                  <p className="text-sm font-semibold">No agents yet</p>
                  <p className="text-sm text-muted-foreground mt-0.5">Create your first AI agent to automate recruiting workflows.</p>
                </div>
                <Button size="sm" className="mt-1" onClick={() => setWizardOpen(true)}><Plus className="size-3.5 mr-1.5" />Create Agent</Button>
              </div>
            ) : (
