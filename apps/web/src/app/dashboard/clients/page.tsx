...
import { mapClientRow, type Client } from './_data'
  ...
  try {
    ...
    if (membership) {
      const [{ data: clientRows }, { data: jobRows }] = await Promise.all([
        admin.from('clients').select('*').eq('tenant_id', membership.tenant_id).is('deleted_at', null).order('created_at', { ascending: false }),
        admin.from('jobs').select('client, status').eq('tenant_id', membership.tenant_id).is('deleted_at', null),
      ])
