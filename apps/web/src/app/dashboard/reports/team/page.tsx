...
const REPORT_CARDS = [
  { name: 'Team Productivity', desc: 'Overall team output and efficiency' },
  { name: 'Team Leaderboard', desc: 'Ranked performance across all team members' },
  { name: 'Jobs per Recruiter', desc: 'Active job load per recruiter' },
  { name: 'Submissions per Recruiter', desc: 'Submission volume by team member' },
  { name: 'Placements per Recruiter', desc: 'Placement counts across the team' },
  { name: 'Average Time to Fill', desc: 'Team average days from open to placement' },
  { name: 'Offer Acceptance Rate', desc: 'Percentage of offers accepted' },
  { name: 'Pipeline Health', desc: 'Stage distribution health check' },
]
...
const INSIGHTS = [
  'Team submission rate is up 14% month-over-month — strong Q2 performance.',
  '2 team members have not reached their weekly submission target.',
  'Offer acceptance rate is 78% — above the 70% industry benchmark.',
  'Average time to fill dropped to 18 days — down from 20 days last quarter.',
]
    ...
    <div className="flex flex-col h-full p-6 gap-4 overflow-y-auto">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <select className="h-8 text-sm border rounded-md px-2 bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]"><option>Last 30 Days</option></select>
