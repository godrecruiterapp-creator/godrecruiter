        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
          ...
          <div>
            ...
            {filters.map((f, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                <select className="h-7 text-sm border rounded-md px-1.5 bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" value={f.field} onChange={(e) => updateFilter(i, { field: e.target.value })}>
                  {cols.map((c) => <option key={c}>{c}</option>)}
                </select>
                <select className="h-7 text-sm border rounded-md px-1.5 bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" value={f.operator} onChange={(e) => updateFilter(i, { operator: e.target.value })}>
                  {OPERATORS.map((o) => <option key={o}>{o}</option>)}
                </select>
                <input className="flex-1 h-7 px-2 text-sm border rounded-md bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" value={f.value} onChange={(e) => updateFilter(i, { value: e.target.value })} placeholder="Value" />
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select className="h-7 text-sm border rounded-md px-1.5 bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
            ...
            <div className="flex items-center gap-2">
              ...
              <select className="h-7 text-sm border rounded-md px-1.5 bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                <option value="">None</option>
                {cols.map((c) => <option key={c}>{c}</option>)}
              </select>
              <select className="h-7 text-sm border rounded-md px-1.5 bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" value={sortDir} onChange={(e) => setSortDir(e.target.value)}>
                <option>ASC</option>
                <option>DESC</option>
              </select>
            </div>
