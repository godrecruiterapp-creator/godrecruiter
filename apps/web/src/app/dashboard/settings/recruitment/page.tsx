      ...
      {adding ? (
        <div className="flex gap-2">
          <input autoFocus value={val} onChange={e => setVal(e.target.value)}
            ...
            className="flex-1 h-8 px-2 text-sm rounded-lg border border-input bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" placeholder="Stage name…" />
  ...
  return (
    <div className="flex items-center flex-wrap gap-1.5">
      ...
      <input value={val} onChange={e => setVal(e.target.value)}
        ...
        placeholder="Add tag…"
        className="h-7 w-28 px-2 text-xs rounded-full border border-dashed border-input bg-background focus-visible:outline-none focus-visible:border-[#D1D5DB]" />
    </div>
  )
}
