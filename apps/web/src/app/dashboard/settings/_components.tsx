    ...
    <div className="sticky bottom-0 left-0 right-0 border-t bg-background/95 backdrop-blur z-10">
      <div className="flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onDiscard} className="h-8 px-3 text-sm rounded-md border border-border hover:bg-muted/60 transition-colors flex items-center gap-1.5">
            <X className="size-3.5" />Discard
          </button>
          <button onClick={onSave} className="h-8 px-3 text-sm rounded-md bg-foreground text-background hover:bg-foreground/90 transition-colors flex items-center gap-1.5">
    ...
    <div className="flex items-center gap-1 mb-5 border-b border-border">
      {tabs.map(t => (
        <button key={t} onClick={() => onChange(t)}
          className={cn('px-3 py-2 text-sm font-medium transition-colors border-b-2 -mb-px', t === active ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground')}>
          {t}
        </button>
      ))}
    </div>
...
export function Badge({ children, variant = 'default' }: {
  ...
  return (
    <span className={cn('inline-flex items-center px-2 py-0.5 rounded-full border text-xs font-medium whitespace-nowrap', {
      'bg-muted/60 text-muted-foreground border-border':         variant === 'default',
      'bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800':  variant === 'success',
      'bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800':    variant === 'warning',
      'bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800':      variant === 'danger',
      'bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800':     variant === 'info',
    })}>
      {children}
    </span>
  )
...
export function Breadcrumb({ label = 'Settings', href = '/dashboard/settings' }: { label?: string; href?: string }) {
  return (
    <Link href={href} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-6 group">
        ...
        <div className="flex items-center gap-2 shrink-0 ml-4">
          ...
          {!editing && (
            <button onClick={() => setEditing(true)}
              className="h-8 px-3.5 text-sm rounded-lg border border-border hover:bg-muted/60 transition-colors font-medium">
              Edit
            </button>
          )}
        </div>
        ...
        <>
          ...
          <div className="flex justify-end gap-2 px-6 py-4 border-t border-border bg-muted/20">
            <button onClick={() => setEditing(false)}
              className="h-8 px-3.5 text-sm rounded-lg border border-border hover:bg-muted/60 transition-colors">
              Cancel
            </button>
            <button onClick={() => setEditing(false)}
              className="h-8 px-3.5 text-sm rounded-lg bg-foreground text-background hover:bg-foreground/90 transition-colors font-medium">
              Save changes
            </button>
          </div>
        </>
