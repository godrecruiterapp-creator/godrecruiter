...
import { Plus, Briefcase, ChevronDown, X, SlidersHorizontal, Trash2 } from 'lucide-react'
...
export function JobsTableClient({ jobs }: { jobs: Job[] }) {
  ...
  function toggleAll(checked: boolean) {
    ...
    else setSelected(prev => { const s = new Set(prev); allIds.forEach(id => s.delete(id)); return s })
  }
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 bg-brand-muted border border-brand/20 rounded-lg shrink-0">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="sm" disabled={isPending}>Manage <ChevronDown className="size-3.5 ml-1.5" /></Button>
            </DropdownMenuTrigger>
          ...
          <Table>
            <TableHeader className="sticky top-0 z-10 bg-muted/60 backdrop-blur-sm">
              <TableRow className="hover:bg-transparent border-b">
                ...
                <TableHead className="w-[105px] text-xs font-semibold">Job ID</TableHead>
                <TableHead className="min-w-[180px] text-xs font-semibold">Job Title</TableHead>
                <TableHead className="min-w-[130px] text-xs font-semibold">Client</TableHead>
                <TableHead className="min-w-[100px] text-xs font-semibold">City</TableHead>
                <TableHead className="min-w-[90px] text-xs font-semibold">State</TableHead>
                <TableHead className="min-w-[110px] text-xs font-semibold">Emp. Type</TableHead>
                <TableHead className="min-w-[90px] text-xs font-semibold">Status</TableHead>
                <TableHead className="min-w-[90px] text-xs font-semibold">Priority</TableHead>
                <TableHead className="min-w-[130px] text-xs font-semibold">Recruiter</TableHead>
                <TableHead className="w-[80px] text-xs font-semibold text-center">Openings</TableHead>
                <TableHead className="w-[90px] text-xs font-semibold text-center">Submitted</TableHead>
                <TableHead className="w-[90px] text-xs font-semibold text-center">Interviews</TableHead>
                <TableHead className="w-[70px] text-xs font-semibold text-center">Offers</TableHead>
                <TableHead className="min-w-[95px] text-xs font-semibold">Created</TableHead>
                <TableHead className="min-w-[95px] text-xs font-semibold">Modified</TableHead>
                <TableHead className="w-[70px] text-xs font-semibold text-center">Aging</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.length === 0 ? (
                ...
                return (
                  <TableRow key={job.id} className={isSel ? 'bg-brand-muted hover:bg-brand-muted' : 'hover:bg-muted/30'}>
                    <TableCell className="w-10 px-3" onClick={e => e.stopPropagation()}>
                      <Checkbox checked={isSel} onCheckedChange={c => toggleOne(job.id, !!c)} aria-label={`Select ${job.title}`} />
                    </TableCell>
