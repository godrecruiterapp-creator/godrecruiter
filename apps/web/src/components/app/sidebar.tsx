...
import {
  ...
  BarChart3, ChevronLeft, ChevronRight, Bot, Zap, FolderKanban, UserCheck, Building2,
} from 'lucide-react'
        ...
        <div className="flex items-center justify-between h-12 border-b px-3 flex-shrink-0">
          {!collapsed && (
            <div className="flex items-center gap-2 min-w-0">
              <img src="/logo-icon-square.png" alt="God Recruiter" className="size-9 object-contain flex-shrink-0" />
              <span className="font-semibold text-base tracking-tight truncate">God Recruiter</span>
            </div>
          )}
          {collapsed && (
            <img src="/logo-icon-square.png" alt="God Recruiter" className="size-9 object-contain mx-auto" />
          )}
          ...
          {NAV.map((entry, i) => {
            ...
            const item = (
              <Link
                ...
                href={href}
                className={cn(
                  'group flex items-center gap-2.5 rounded-md text-sm leading-5 font-normal transition-colors',
              ...
              >
                <Icon className="size-4 flex-shrink-0 transition-transform duration-200 ease-out group-hover:scale-110 group-hover:-translate-y-0.5" />
